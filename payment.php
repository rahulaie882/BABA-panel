...
<head>
    ...
    <title>Payment Setup - BABA PANEL</title>
    <style>
...
<body>
    <div class="header"><h1>💳 UPI & QR Control</h1><div>👤 <?= htmlspecialchars($_SESSION['admin_user']) ?></div></div>
    <div class="container">
        <a href="dashboard.php" class="back-link">← Back to Dashboard</a>
        <?php if($success): ?><div class="alert"><?= $success ?></div><?php endif; ?>
        <div class="card">
            <form method="POST" enctype="multipart/form-data">
                <label>Merchant UPI ID Control</label>
                <input type="text" name="upi_id" value="<?= htmlspecialchars($s['upi_id'] ?? '') ?>" placeholder="yourname@upi" required>

                <label>Payment QR Code Control</label>
